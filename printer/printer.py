...
printer = gdb.printing.RegexpCollectionPrettyPrinter('AntDB')
...
def node_type(val):
    type = str(val['type'])[2:]
    if type in ['Integer', 'Float', 'BitString', 'String']:
        return 'Value'
       
    if type == 'Null':
        return 'A_Const'
        
    if type in ['OidList', 'IntList']:
        return 'List'
...
class ValuePrinter(Printer):
    def to_string(self):
        ...
        elif vt in ['Float', 'BitString', 'String']:
...
class A_ConstPrinter(Printer):
    def to_string(self):
        if str(self.val['type'])[2:] == 'Null':
            return 'Null'

        return self.val['val'].dereference()
    ...
    class _iter(object):
        ...
        def __next__(self):
            ...
            if str(self.type) == 'List':
                i_type = cast2ptr(node['ptr_value'], 'Node')['type']
                type = node_type(cast2ptr(node['ptr_value'], 'Node'))
                if int(i_type) > 0 and int(i_type) < gdb.parse_and_eval('(int) T_SupportRequestIndexCondition'):
                    node = cast2ptr(node, str(type)).dereference()
                else:
                    node = node['ptr_value']
...
class Gnerator:
    def __init__(self) -> None:
        self.printer_nodes = [
            'IndexInfo', 'ExprContext', 'ProjectionInfo', 'JunkFilter', 'OnConflictSetState', 'ResultRelInfo', 'EState', 'TupleTableSlot',
            'NestLoopParam', 'PlanRowMark', 'PartitionPruneInfo', 'PartitionedRelPruneInfo', 'PartitionPruneStepOp', 'PartitionPruneStepCombine', 'PlanInvalItem', 'ExecNodes', 'SimpleSort', 'RemoteQuery', 'AlterNodeStmt',
            'CreateNodeStmt', 'DropNodeStmt', 'CreateGroupStmt', 'DropGroupStmt', 'CreateAuxStmt', 'PaddingAuxDataStmt',
            'EmptyResult', 'ParamTuplestoreScan', 'PlanState', 'ResultState', 'ProjectSetState', 'ModifyTableState', 'AppendState', 'MergeAppendState', 'RecursiveUnionState',
            'BitmapAndState', 'BitmapOrState', 'ScanState', 'SeqScanState', 'SampleScanState', 'IndexScanState', 'IndexOnlyScanState', 'BitmapIndexScanState', 'BitmapHeapScanState', 'TidScanState',
            'SubqueryScanState', 'FunctionScanState', 'TableFuncScanState', 'ValuesScanState', 'CteScanState', 'NamedTuplestoreScanState', 'WorkTableScanState', 'ForeignScanState',
            'CustomScanState', 'JoinState', 'NestLoopState', 'MergeJoinState', 'HashJoinState', 'MaterialState', 'SortState', 'IncrementalSortState', 'GroupState',
            'AggState', 'WindowAggState', 'UniqueState', 'GatherState', 'GatherMergeState', 'HashState', 'SetOpState', 'LockRowsState', 'LimitState',
            'BatchSortState', 'RemoteCopyState', 'RemoteQueryState', 'ClusterGatherState', 'ClusterMergeGatherState', 'ClusterReduceState', 'ReduceScanState', 'EmptyResultState',
            'ParamTuplestoreScanState', 'ConnectByState', 'Alias', 'RangeVar', 'TableFunc', 'Var', 'Const', 'Param',
            'Aggref', 'GroupingFunc', 'WindowFunc', 'SubscriptingRef', 'FuncExpr', 'NamedArgExpr', 'OpExpr', 'DistinctExpr', 'NullIfExpr',
            'ScalarArrayOpExpr', 'BoolExpr', 'SubLink', 'SubPlan', 'AlternativeSubPlan', 'FieldSelect', 'FieldStore',
            'RelabelType', 'CoerceViaIO', 'ArrayCoerceExpr', 'ConvertRowtypeExpr', 'CollateExpr', 'CaseExpr', 'CaseWhen', 'CaseTestExpr', 'ArrayExpr',
            'RowExpr', 'RowCompareExpr', 'CoalesceExpr', 'MinMaxExpr', 'SQLValueFunction', 'XmlExpr', 'NullTest', 'BooleanTest',
            'CoerceToDomain', 'CoerceToDomainValue', 'SetToDefault', 'CurrentOfExpr', 'NextValueExpr', 'RownumExpr', 'LevelExpr', 'SysConnectByPathExpr',
            'ConnectByRootExpr', 'OracleConnectBy', 'InferenceElem', 'TargetEntry', 'RangeTblRef', 'JoinExpr', 'FromExpr', 'OnConflictExpr', 'IntoClause',
            'PGXCSubCluster', 'ExprState', 'AggrefExprState', 'WindowFuncExprState', 'SetExprState', 'SubPlanState', 'AlternativeSubPlanState', 'DomainConstraintState',
            'PlannerInfo', 'PlannerGlobal', 'RelOptInfo', 'IndexOptInfo', 'ForeignKeyOptInfo', 'ParamPathInfo',
            'EquivalenceClass', 'EquivalenceMember', 'PathKey', 'PathTarget', 'RestrictInfo', 'IndexClause', 'PlaceHolderVar', 'SpecialJoinInfo', 'AppendRelInfo', 'PlaceHolderInfo',
            'MinMaxAggInfo', 'PlannerParamItem', 'RollupData', 'GroupingSetData', 'StatisticExtInfo', 'MemoryContext', 'AllocSetContext', 'SlabContext',
            'GenerationContext',
            'ExtensibleNode', 'RawStmt', 'Query', 'PlannedStmt', 'InsertStmt', 'DeleteStmt', 'UpdateStmt', 'SelectStmt',
            'AlterTableStmt', 'AlterTableCmd', 'AlterDomainStmt', 'SetOperationStmt', 'GrantStmt', 'GrantRoleStmt', 'AlterDefaultPrivilegesStmt', 'ClosePortalStmt',
            'ClusterStmt', 'CopyStmt', 'CreateStmt', 'DefineStmt', 'DropStmt', 'TruncateStmt', 'CommentStmt', 'FetchStmt', 'IndexStmt', 'CreateFunctionStmt',
            'AlterFunctionSmt','DoStmt','RenameStmt','RuleStmt','NotifyStmt','ListenStmt','UnlistenStmt','TransactionStmt','ViewStmt',
            'LoadStmt', 'CreateDomainStmt', 'CreatedbStmt', 'DropdbStmt', 'VacuumStmt', 'ExplainStmt', 'CreateTableAsStmt', 'CreateSeqStmt',
            'AlterSeqStmt', 'VariableSetStmt', 'VariableShowStmt', 'DiscardStmt', 'CreateTrigStmt', 'CreatePLangStmt', 'CreateRoleStmt', 'AlterRoleStmt', 'DropRoleStmt', 'LockStmt',
            'ConstraintsSetStmt', 'ReindexStmt', 'CheckPointStmt', 'CreateSchemaStmt', 'AlterDatabaseStmt', 'AlterDatabaseSetStmt', 'AlterRoleSetStmt', 'CreateConversionStmt', 'CreateCastStmt', 'CreateOpClassStmt',
            'CreateOpFamilyStmt', 'AlterOpFamilyStmt', 'PrepareStmt', 'ExecuteStmt', 'DeallocateStmt', 'DeclareCursorStmt', 'CreateTableSpaceStmt', 'DropTableSpaceStmt',
            'AlterObjectDependsStmt', 'AlterObjectSchemaStmt', 'AlterOwnerStmt', 'AlterOperatorStmt', 'AlterTypeStmt', 'DropOwnedStmt', 'ReassignOwnedStmt', 'CompositeTypeStmt', 'CreateEnumStmt', 'CreateRangeStmt', 'AlterEnumStmt', 'AlterTSDictionaryStmt',
            'AlterTSConfigurationStmt', 'CreateFdwStmt', 'AlterFdwStmt', 'CreateForeignServerStmt', 'AlterForeignServerStmt', 'CreateUserMappingStmt', 'AlterUserMappingStmt', 'DropUserMappingStmt', 'ExecDirectStmt',
            'CleanConnStmt', 'FinishActiveBackendStmt', 'AlterTableSpaceOptionsStmt', 'AlterTableMoveAllStmt', 'SecLabelStmt', 'CreateForeignTableStmt', 'ImportForeignSchemaStmt', 'CreateExtensionStmt', 'AlterExtensionStmt', 'AlterExtensionContentsStmt',
            'CreateEventTrigStmt', 'AlterEventTrigStmt', 'RefreshMatViewStmt', 'ReplicaIdentityStmt', 'AlterSystemStmt', 'CreatePolicyStmt', 'AlterPolicyStmt', 'CreateTransformStmt', 'CreateAmStmt',
            'CreatePublicationStmt', 'AlterPublicationStmt', 'CreateSubscriptionStmt', 'AlterSubscriptionStmt', 'DropSubscriptionStmt', 'CreateStatsStmt', 'AlterCollationStmt', 'CallStmt',
            'AlterStatsStmt', 'CreateOracleConvertStmt', 'CreateOracleCastStmt', 'CreateSynonymStmt', 'DropSynonymStmt', 'BarrierStmt', 'AdbLicenseStmt', 'A_Expr',
            'ColumnRef', 'ColumnRefJoin', 'PriorExpr', 'PackageStmt', 'KeepClause', 'ParamRef',  'FuncCall',
            'A_Indices', 'A_Indirection', 'A_ArrayExpr', 'ResTarget', 'MultiAssignRef', 'TypeCast', 'CollateClause',
            'SortBy', 'WindowDef', 'RangeSubselect', 'RangeFunction', 'RangeTableSample', 'RangeTableFunc', 'RangeTableFuncCol', 'TypeName', 'ColumnDef',
            'IndexElem', 'Constraint', 'DefElem', 'RangeTblEntry', 'RangeTblFunction', 'TableSampleClause', 'WithCheckOption', 'SortGroupClause',
            'GroupingSet', 'WindowClause', 'ObjectWithArgs', 'AccessPriv', 'CreateOpClassItem', 'TableLikeClause', 'FunctionParameter',
            'LockingClause', 'RowMarkClause', 'XmlSerialize', 'WithClause', 'InferClause', 'OnConflictClause', 'CommonTableExpr', 'RoleSpec',
            'TriggerTransition', 'PartitionElem', 'PartitionSpec', 'PartitionBoundSpec', 'PartitionRangeDatum', 'PartitionCmd', 'VacuumRelation',
            'IdentifySystemCmd', 'BaseBackupCmd', 'CreateReplicationSlotCmd', 'DropReplicationSlotCmd', 'StartReplicationCmd', 'TimeLineHistoryCmd',
            'SQLCmd', 'UltraSyncInfoCmd', 'TriggerData', 'EventTriggerData', 'ReturnSetInfo', 'WindowObjectData', 'TIDBitmap',
            'InlineCodeBlock', 'FdwRoutine', 'IndexAmRoutine', 'TableAmRoutine', 'TsmRoutine', 'ForeignKeyCacheInfo',
            'CallContext',
            'SupportRequestSimplify', 'SupportRequestSelectivity', 'SupportRequestCost', 'SupportRequestRows', 'SupportRequestIndexCondition']
        ...
        self.path_nodes = [
            ...
            'IncrementalSortPath', 'BatchSortPath', 'RemoteQueryPath', 'ClusterGatherPath', 'ClusterMergeGatherPath', 'ClusterReducePath', 'ReduceScanPath',
            'FilterPath', 'ConnectByPath'
        ]
        ...
        self.plan_nodes = [
            ...
            'NamedTuplestoreScan', 'WorkTableScan', 'ForeignScan', 'CustomScan', 'NestLoop',  'MergeJoin', 'HashJoin', 'Material', 'Memoize', 'ConnectByPlan',
            'Sort', 'IncrementalSort', 'Group', 'Agg', 'WindowAgg', 'Unique', 'Gather', 'GatherMerge', 'Hash', 'SetOp', 'LockRows', 'Limit', 'BatchSort',
            'ClusterGather', 'ClusterMergeGather', 'ClusterReduce', 'ReduceScan',
        ]
