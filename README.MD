...
* node_struct 是一堆字典，保存是postgres 中得主要结构体
* 使用此信息，使用generate_printer批量实现结构体的注册
* 部分结构体需要自行管理，类似list，bitmap等
* 部分结构体需要主要防止递归解析，如不同结构体之间相互应用，典型例子为等价类
* 实现不同等级
  * 直接打印全部信息，类似pprint
  * 简化打印信息，会去掉一些不重要的信息，例如以树的形式打印path，plan
  * 使用部分函数，在上面的基础上简化部分信息，例如常量的打印，类型的解析
