...
pl = {
    ...
    # gp
    'CdbMotionPath': CdbMotionPath,    'DistributionKey': DistributionKey,    'AOCSPath': AOCSPath,    'AppendOnlyPath': AppendOnlyPath,
    'CdbPathLocus': CdbPathLocus,       'Motion': Motion,   'SplitUpdate': SplitUpdate,  'AssertOp': AssertOp,  'Flow': Flow,  'PlanSlice': PlanSlice,
    'DirectDispatchInfo': DirectDispatchInfo, 'RowIdExpr': RowIdExpr,
 }
...
@register_printer('GpPolicy')
class GpPolicyPrinter(BasePrinter):
    def to_string(self):
        return self.print_to_string('GpPolicy', [('GpPolicyType', 'ptype'), ('int', 'numsegments')])

    def children(self):
        list = []
        nattrs = int(self.val['nattrs'])
        if nattrs != 0:
            list.append(('attrs', str([int(self.val['attrs'][i]) for i in range(nattrs)])))
            list.append(('opclasses', str([int(self.val['opclasses'][i]) for i in range(nattrs)])))
